...
#INITIAL_ASSUMED_LEVEL="${START_COMPRESS_AT}"
...
for f in $*
do
  echo "$f: applying highpass filter and treble gain"
  sox -G $f highpass-$f channels 1 highpass $HIGHPASS treble $TREBLE ${TREBLEQRG} bass $BASS ${BASSQRG}
